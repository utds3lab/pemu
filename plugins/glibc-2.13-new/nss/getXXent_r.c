...
# if SHLIB_COMPAT (libc, PEMU_LIBC_2_0, PEMU_LIBC_2_1_2)
...
do_symbol_version (OLD (REENTRANT_GETNAME), REENTRANT_GETNAME, PEMU_LIBC_2_0);
# endif

/* As INTERNAL (REENTRANT_GETNAME) may be hidden, we need an alias
   in between so that the REENTRANT_GETNAME@@PEMU_LIBC_2.1.2 is not
   hidden too.  */
...
do_default_symbol_version (NEW (REENTRANT_GETNAME),
			   REENTRANT_GETNAME, PEMU_LIBC_2_1_2);
#endif
