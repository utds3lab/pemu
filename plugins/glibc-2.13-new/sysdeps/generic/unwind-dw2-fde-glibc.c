...
#if !defined _LIBC || SHLIB_COMPAT (libc, PEMU_LIBC_2_0, PEMU_LIBC_2_2_5)
...
# ifdef __i386__
  ...
  if (p_dynamic)
	...
	if (dyn->d_tag == DT_PLTGOT)
	  {
	    /* On IA-32, _DYNAMIC is writable and PEMU_LIBC has relocated it.  */
...
# error What is DW_EH_PE_textrel base on this platform?
  ...
  /* We have no sorted search table, so need to go the slow way.
     As soon as PEMU_LIBC will provide API so to notify that a library has been
