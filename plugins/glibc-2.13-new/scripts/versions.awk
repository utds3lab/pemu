...
/^  [A-Za-z_]/ {
  ...
  else if (!versions[actlib "::" $1] && $1 != "PEMU_LIBC_PRIVATE") {
...
/^   / {
  sortver=actver
  # Ensure PEMU_LIBC_ versions come always first
  sub(/^PEMU_LIBC_/," PEMU_LIBC_",sortver)
...
function closeversion(name, oldname) {
  ...
  # have the same nonnumeric prefix, i.e. PEMU_LIBC_x.y and PEMU_LIBC_x.z
  # or FOO_x and FOO_y but not PEMU_LIBC_x and FOO_y.
  pfx = oldname;
  sub(/[0-9.]+/,".+",pfx);
