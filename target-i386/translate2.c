                               ...
                               target_ulong pc_start)
{
	...
	PEMU_read_mem(pc_start, 15, pemu_xed.PEMU_inst_buf);
	xed2_update(pemu_xed.PEMU_inst_buf);
	if(pemu_exec_stats.PEMU_start)
	    //if(pemu_exec_stats.PEMU_start != 0
	    //		&& pemu_exec_stats.PEMU_cr3 == PEMU_get_cr3()) //is specified process?
	    pemu_hook_funcs.inst_hook(pemu_xed.PEMU_xedd_g, 0);
    }
#endif
                                                  ...
                                                  int search_pc)
{
	...
	if(pemu_exec_stats.PEMU_start != 0 
	    ...
	    //pemu_hook_funcs.bbl_hook(pemu_xed.PEMU_xedd_g, 0);
	}
#endif
	target_ulong next_pc;
	if(pemu_exec_stats.PEMU_start != 0) {
		//start_code_cache(tb->pc);
		next_pc = disas_basic_block(pemu_bbl.bbl, tb->pc);
	}
	//end
    gen_tb_start();
    for(;;) {
        ...
        if (dc->is_jmp){
			//printf("jmp comes:%x\t%x\n", pc_ptr, tb->flags & HF_INHIBIT_IRQ_MASK);
            break;
		}
        ...
        if (dc->tf || dc->singlestep_enabled ||
            ...
            gen_eob(dc);
			if(tb->pc < 0xc0000000){
				//printf("irq comes:%x\t%x\t%x\t%x\t%x\n", tb->pc, pc_ptr, dc->tf, dc->singlestep_enabled, (flags & HF_INHIBIT_IRQ_MASK));
			}
            break;
        }
    ...
    *tcg_ctx.gen_opc_ptr = INDEX_op_end;
	...
	//jzeng
	if(pemu_exec_stats.PEMU_start != 0 && next_pc != -1) {
